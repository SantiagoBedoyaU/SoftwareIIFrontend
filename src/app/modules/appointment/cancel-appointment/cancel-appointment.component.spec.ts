...
describe('CancelAppointmentComponent', () => {
  ...
  const mockM = {
    ...
    Datepicker: {
      init: jasmine.createSpy('init').and.callFake((elem: any, options: any) => ({
        open: jasmine.createSpy('open'),
        close: jasmine.createSpy('close'),
      })),
    },
  };
  (window as any).M = mockM;
  ...
  it('should set doctor_name to "Nombre no disponible" when doctor_id is null or empty', () => {
    ...
    mockAppointmentService.getAppointmentsByPatient.and.returnValue(of(appointments));
    mockSecurityService.getUserByDNI.and.returnValue(of(null)); // Este valor no debería importar porque doctor_id es null.
  ...
  it('should open the datepicker when the date icon is clicked', () => {
    // Crear un contenedor para simular la jerarquía del DOM
    ...
    // Establecer relaciones DOM entre los elementos
    parentDiv.appendChild(dateInput);
    parentDiv.appendChild(dateIcon);
  
    // Espiar en querySelectorAll para devolver los elementos simulados
    spyOn(document, 'querySelectorAll').and.returnValue([dateInput] as unknown as NodeListOf<Element>);
  
    // Espiar en addEventListener del ícono para capturar el evento de clic
    const addEventListenerSpy = spyOn(dateIcon, 'addEventListener').and.callFake((event: string, handler: EventListener) => {
      if (event === 'click') {
        // Simula el clic llamando a la función handler
        (handler as EventListener)(new Event('click'));
      }
    });
  
    // Espiar en M.Datepicker.init para simular el comportamiento del selector de fecha
    const pickerMock = {
      open: jasmine.createSpy('open'),
    };
    (window as any).M.Datepicker.init.and.returnValue(pickerMock);
  
    // Llamar al método que inicializa los datepickers
    component.initDatepickers();
  
    // Verificar que addEventListener se haya llamado en el ícono
    expect(addEventListenerSpy).toHaveBeenCalledWith('click', jasmine.any(Function));
  
    // Verificar que el método open del picker haya sido llamado
  ...
  it('should set startDate when the first Datepicker closes', () => {
    ...
    const datePickerOptions = (window as any).M.Datepicker.init.calls.mostRecent().args[1];
  ...
  it('should set endDate when the second Datepicker closes', () => {
    const dateInputs = [document.createElement('input'), document.createElement('input')].map(
      (input) => input as HTMLInputElement
    );
    dateInputs[1].classList.add('datepicker');
    dateInputs[1].value = '2024-12-15';
    spyOn(document, 'querySelectorAll').and.returnValue(dateInputs as unknown as NodeListOf<Element>);
  ...
  it('should update the modal message if a message is provided', () => {
    ...
    const modalSpy = (globalThis as any).M.Modal.getInstance(modalElement);
    ...
    document.body.removeChild(modalElement); // Limpia el DOM después de la prueba
  });
  ...
  it('should initialize datepickers', () => {
    component.initDatepickers();
    expect((globalThis as any).M.Datepicker.init).toHaveBeenCalled();
  });
  ...
  it('should show modal on cancel appointment', () => {
    const modalSpy = (globalThis as any).M.Modal.getInstance();
  ...
  it('should open confirmation modal before canceling an appointment', () => {
   ...
   it('should cancel an appointment successfully', () => {
    ...
    component.appointments = [{ id: '1', start_date: '2024-11-23T10:00:00', doctor_id: '123', doctor_name: 'John Doe' }];
  ...
  it('should show a modal', () => {
    ...
    spyOn(document, 'getElementById').and.returnValue(modalElement); // Simula el modal en el DOM
    const modalSpy = (globalThis as any).M.Modal.getInstance(modalElement);
